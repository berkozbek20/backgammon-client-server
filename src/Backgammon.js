...
const Dice = ({ value, position, rolling }) => {
  ...
  useFrame((state, delta) => {
    ...
    if (rolling) {
       ...
       ref.current.position.y = THREE.MathUtils.lerp(ref.current.position.y, 0.30, delta*10);
  ...
  return (
    <group position={position}>
       <group ref={ref}>
         <RoundedBox args={[0.60, 0.60, 0.60]} radius={0.1} castShadow>
            ...
            <group rotation={[Math.PI/2, 0, 0]}><Pip pos={[0, 0.31, -0.15]} /><Pip pos={[0, 0.31, 0.15]} /></group>
            <group rotation={[-Math.PI/2, 0, 0]}><Pip pos={[-0.15, 0.31, -0.15]} /><Pip pos={[0.15, 0.31, 0.15]} /><Pip pos={[0, 0.31, 0]} /><Pip pos={[-0.15, 0.31, 0.15]} /><Pip pos={[0.15, 0.31, -0.15]} /></group>
            <group rotation={[0, 0, -Math.PI/2]}><Pip pos={[-0.15, 0.31, -0.15]} /><Pip pos={[0, 0.31, 0]} /><Pip pos={[0.15, 0.31, 0.15]} /></group>
            <group rotation={[0, 0, Math.PI/2]}><Pip pos={[-0.15, 0.31, -0.15]} /><Pip pos={[0.15, 0.31, -0.15]} /><Pip pos={[-0.15, 0.31, 0.15]} /><Pip pos={[0.15, 0.31, 0.15]} /></group>
         </RoundedBox>
       </group>
    </group>
  );
...
const Checker = ({ color, position, isTopStack, onDragStart, opacity = 1 }) => {
  ...
  return (
    <group 
        position={position}
        onPointerOver={(e) => { e.stopPropagation(); if(isTopStack) setHover(true); }}
    ...
    >
      ...
      {/* İç Desen */}
      {opacity === 1 && (
          <mesh position={[0, BOARD_SPECS.checkerHeight/2 + 0.001, 0]} rotation={[-Math.PI/2,0,0]}>
            <ringGeometry args={[BOARD_SPECS.checkerRadius*0.4, BOARD_SPECS.checkerRadius*0.7, 32]} />
            <meshStandardMaterial color={isWhite ? '#ddd' : '#333'} transparent opacity={0.3} />
          </mesh>
      )}
    </group>
  );
...
const GameScene = ({ isInteractive }) => {
  ...
  const [boardState, setBoardState] = useState(gameRef.current.getBoardSnapshot());
  ...
  const handleRoll = () => {
    ...
    setTimeout(() => {
        ...
        if (allLegalMoves.length === 0) {
            const pl = gameRef.current.getCurrentPlayer() === Player.WHITE ? "BEYAZ" : "SİYAH";
            ...
            setTimeout(() => {
                setVisualDice([]);
                setRemainingMoves([]);
                const nextPl = gameRef.current.getCurrentPlayer() === Player.WHITE ? "BEYAZ" : "SİYAH";
                setMsg(`Sıra: ${nextPl}`);
            }, 2500); 
        } else {
            const pl = gameRef.current.getCurrentPlayer() === Player.WHITE ? "BEYAZ" : "SİYAH";
  ...
  const onPlanePointerMove = (e) => {
    if(dragging) {
        ...
        for(let t of targets) {
            ...
            const dist = Math.sqrt(Math.pow(e.point.x - info.x, 2) + Math.pow(e.point.z - info.zBase, 2));
  ...
  const onPointerUp = (e) => {
    ...
    if (dropTarget !== null && dragItem) {
        ...
        if (bestIdx === 'off') {
             const distanceNeeded = gameRef.current.getCurrentPlayer() === Player.WHITE 
                ? 24 - dragItem.index 
                : dragItem.index - (-1);
  ...
  const curPlayerName = gameRef.current.getCurrentPlayer() === Player.WHITE ? "BEYAZ" : "SİYAH";
  ...
  return (
    <>
      ...
      <group onPointerUp={dragging ? null : onPointerUp}>
        {/* Tahta Gövdesi */}
        <RoundedBox args={[BOARD_SPECS.width + 1.2, 1.2, BOARD_SPECS.height + 1.2]} position={[0, -0.62, 0]} radius={0.2} receiveShadow>
        ...
        {/* Çerçeve Kenarları */}
        <RoundedBox args={[BOARD_SPECS.width + 1.2, 0.4, 0.6]} position={[0, 0.2, -BOARD_SPECS.height/2 - 0.3]} radius={0.1}><meshStandardMaterial color={COLORS.frameRim} /></RoundedBox>
        <RoundedBox args={[BOARD_SPECS.width + 1.2, 0.4, 0.6]} position={[0, 0.2, BOARD_SPECS.height/2 + 0.3]} radius={0.1}><meshStandardMaterial color={COLORS.frameRim} /></RoundedBox>
        <RoundedBox args={[0.6, 0.4, BOARD_SPECS.height]} position={[-BOARD_SPECS.width/2 - 0.3, 0.2, 0]} radius={0.1}><meshStandardMaterial color={COLORS.frameRim} /></RoundedBox>
        <RoundedBox args={[0.6, 0.4, BOARD_SPECS.height]} position={[BOARD_SPECS.width/2 + 0.3, 0.2, 0]} radius={0.1}><meshStandardMaterial color={COLORS.frameRim} /></RoundedBox>
        <mesh position={[0, 0.05, 0]}><boxGeometry args={[BOARD_SPECS.barWidth, 0.15, BOARD_SPECS.height]} /><meshStandardMaterial color={COLORS.frameMain} /></mesh>
        ...
        {dragging && dragItem && (
            <Checker color={dragItem.owner} position={dragPos} isTopStack={false} />
        )}
      </group>

      {/* 2D UI Katmanı (HTML Overlay) */}
      <Html fullscreen style={{ pointerEvents: 'none' }}>
        <div style={{ position: 'absolute', top: '25px', left: '30px', pointerEvents: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '8px' }}>
             <div style={{ display:'flex', alignItems:'center', gap:'12px', background:'rgba(0,0,0,0.6)', padding:'8px 16px', borderRadius:'12px', border:'1px solid rgba(255,255,255,0.1)' }}>
                 <div style={{ fontSize: '32px' }}>🎲</div>
                 <div style={{ display:'flex', flexDirection:'column' }}>
                     <span style={{ color:'white', fontWeight:'bold', fontSize:'24px', letterSpacing:'1px', textTransform:'uppercase' }}>TAVLA OYUNU</span>
                 </div>
             </div>
             <div style={{display:'flex', gap:'5px'}}>
                 <button onClick={handleRestart} style={{ background:'#e74c3c', color:'white', border:'none', padding:'10px 20px', borderRadius:'8px', cursor:'pointer', fontWeight:'bold', fontSize:'14px', boxShadow: '0 2px 5px rgba(0,0,0,0.3)' }}>YENİ OYUN</button>
             </div>
        </div>
        ...
        {remainingMoves.length > 0 && (
          <div style={{
            ...
            <div style={{ fontSize: '14px', marginBottom: '8px', color:'#ddd', textTransform:'uppercase' }}>Kalan Zarlar</div>
            {remainingMoves.map((m, i) => (
              <span key={i} style={{ 
                  display: 'inline-block', width: '36px', height: '36px', lineHeight: '36px', 
                  background: '#f1c40f', color: '#111', 
                  margin: '4px', borderRadius: '6px', fontWeight: '800', fontSize: '20px',
        ...
        <div style={{
            ...
            background: 'rgba(30, 20, 10, 0.85)', backdropFilter: 'blur(10px)', 
            padding: '0', borderRadius: '12px', border: '1px solid rgba(243, 156, 18, 0.3)', 
            color: '#eee', width: '280px', height: '350px', boxShadow: '0 8px 32px rgba(0,0,0,0.5)',
            display: 'flex', flexDirection: 'column', pointerEvents: 'auto', overflow: 'hidden',
            fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif"
        }}>
            <div style={{ padding: '12px 15px', background: 'rgba(243, 156, 18, 0.15)', borderBottom: '1px solid rgba(243, 156, 18, 0.2)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span style={{ fontSize: '18px' }}>💬</span>
                <span style={{ fontWeight: '600', color: '#f39c12', letterSpacing: '0.5px' }}>Oyun Sohbeti</span>
            </div>
            <div style={{ flexGrow: 1, overflowY: 'auto', padding: '15px', display:'flex', flexDirection:'column', gap:'10px' }}>
                <div style={{ alignSelf: 'flex-start', maxWidth: '85%' }}>
                    <div style={{ fontSize: '11px', color: '#aaa', marginBottom: '2px', marginLeft: '4px' }}>Rakip</div>
                    <div style={{ background: 'rgba(255, 255, 255, 0.1)', padding: '8px 12px', borderRadius: '12px 12px 12px 2px', fontSize: '13px', lineHeight: '1.4' }}>Selam! Hazır mısın?</div>
                </div>
                <div style={{ alignSelf: 'flex-end', maxWidth: '85%' }}>
                     <div style={{ background: 'linear-gradient(135deg, #d35400, #e67e22)', color: 'white', padding: '8px 12px', borderRadius: '12px 12px 2px 12px', fontSize: '13px', lineHeight: '1.4', boxShadow: '0 2px 5px rgba(0,0,0,0.2)' }}>Her zaman! Bol şans.</div>
                </div>
            </div>
            <div style={{ padding: '10px', background: 'rgba(0,0,0,0.2)', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', gap: '8px' }}>
                <input type="text" placeholder="Mesaj yazın..." style={{ flexGrow: 1, padding: '10px 12px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white', outline: 'none', fontSize: '13px' }} onFocus={(e) => e.target.style.borderColor = '#f39c12'} onBlur={(e) => e.target.style.borderColor = 'rgba(255,255,255,0.1)'} />
                <button style={{ width: '38px', height: '38px', borderRadius: '50%', border: 'none', background: 'linear-gradient(135deg, #f39c12, #d35400)', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 5px rgba(0,0,0,0.3)', fontSize: '14px' }}>➤</button>
            </div>
        </div>
        ...
        {winner && (
            <div style={{
                ...
                background:'rgba(0,0,0,0.85)', display:'flex', alignItems:'center', justifyContent:'center', pointerEvents:'auto', zIndex: 999
            }}>
                <div style={{textAlign:'center', animation: 'fadeIn 0.5s'}}>
                    <h1 style={{color: winner === Player.WHITE ? '#fff' : '#aaa', fontSize:'60px', marginBottom:'10px', textShadow: '0 0 20px rgba(255,255,255,0.5)'}}>
                        {winner === Player.WHITE ? "BEYAZ KAZANDI! 🎉" : "SİYAH KAZANDI! 🎉"}
                    </h1>
                    <p style={{color: '#ddd', fontSize: '20px', marginBottom: '30px'}}>Harika bir oyundu!</p>
                    <button onClick={handleRestart} style={{
                        padding:'15px 40px', fontSize:'20px', background:'linear-gradient(to right, #e74c3c, #c0392b)', color:'white', border:'none', borderRadius:'50px', cursor:'pointer', boxShadow: '0 5px 15px rgba(231, 76, 60, 0.4)', transition: 'transform 0.2s'
        ...
        <div style={{ position: 'absolute', bottom: '30px', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', pointerEvents: 'none' }}>
          <div style={{
              background: 'rgba(20, 20, 20, 0.9)', color: '#fff', padding: '12px 30px', borderRadius: '50px', pointerEvents: 'auto', textAlign: 'center', boxShadow: '0 10px 25px rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', gap: '20px', border: '1px solid rgba(255,255,255,0.15)', backdropFilter: 'blur(8px)'
            }}>
            <span style={{ fontSize: '1.2em', fontWeight: 'bold', color: '#f39c12', minWidth:'140px' }}>{msg || `Sıra: ${curPlayerName}`}</span>
            <div style={{ width: '1px', height: '25px', background: 'rgba(255,255,255,0.2)' }}></div>
            <button onClick={handleRoll} disabled={isDiceDisabled} style={{
                padding: '10px 30px', background: isDiceDisabled ? '#555' : 'linear-gradient(135deg, #f39c12, #d35400)', color: isDiceDisabled ? '#aaa' : '#fff', border: 'none', borderRadius: '25px', cursor: isDiceDisabled ? 'default' : 'pointer', fontWeight: 'bold', fontSize: '16px', letterSpacing:'1px', boxShadow: isDiceDisabled ? 'none' : '0 4px 10px rgba(243, 156, 18, 0.4)', transition: 'all 0.2s ease', opacity: isDiceDisabled ? 0.7 : 1
              }}>ZAR AT</button>
          </div>
        </div>
      </Html>
...
const OpeningCinematic = ({ onComplete }) => {
    ...
    useFrame((state, delta) => {
      if (progress.current < 1) {
        ...
        state.camera.position.set(0, THREE.MathUtils.lerp(8, 20, ease), THREE.MathUtils.lerp(24, 0.1, ease));
